...
Sources of truth:
  ...
  - LOC per module:      walk src/substratecloud/<mod>/*.py
...
def render_report(features: list[dict[str, Any]]) -> str:
    ...
    return f"""# SubstrateCloud SDK — Feature Metrics
...
- **1 line of Python**: `SubstrateCloud().launch(name="x", gpu="A100", image="...")`
- **1 manifest**: `substratecloud apply substratecloud.yaml`
- **1 CLI command**: `substratecloud instance launch --gpu A100 --name x`
...
> Source: live `wc -l` over `src/substratecloud/`, and `__all__` counts in each `__init__.py`.
