...
try:
    ...
    raise SystemExit(
        "The substrate CLI requires the [cli] extras. Install with:\n"
...
def make_client(token: str | None = None, base_url: str | None = None, profile: str | None = None) -> Substrate:
    """Construct a Substrate client with friendly error reporting."""
    try:
        return Substrate(token=token, base_url=base_url, profile=profile)
    except SubstrateError as exc:
        err_console.print(f"[bold red]Configuration error:[/bold red] {exc.message}")
        err_console.print(
            "[dim]Run [bold]substrate config init[/bold] to set up your credentials.[/dim]"
        )
        raise SystemExit(2) from None


def handle_errors(fn: Callable[..., Any]) -> Callable[..., Any]:
    """Decorator: convert SubstrateError into a clean CLI error + exit code."""
    import functools
...
def _exit_code_for(exc: SubstrateError) -> int:
