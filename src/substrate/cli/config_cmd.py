"""`substrate config ...` — interactive token + profile setup."""
...
def init(
    ...
    config_path: Path | None = typer.Option(
        None, "--config", help="Override config file path (defaults to ~/.config/substrate/config.toml)."
    ),
    ...
    console.print(
        "[dim]Your MCP token is created from the Substrate console: "
...
def list_profiles(
    ...
    if not current.profiles:
        console.print("[yellow]No profiles configured. Run `substrate config init`.[/yellow]")
        return
