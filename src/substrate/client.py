"""Top-level Substrate client. The single entry point for SDK users."""
...
from substrate import config as config_module
from substrate._http.auth import resolve_base_url, resolve_token
from substrate._http.client import HttpClient
from substrate._http.logging import get_logger
from substrate._http.retries import RetryPolicy
...
from substrate.resources.instances import InstancesManager
from substrate.resources.inventory import InventoryManager
from substrate.resources.ssh_keys import SSHKeysManager
from substrate.workloads.secret import Secret

if TYPE_CHECKING:
    from substrate.workloads.base import Workload

_log = get_logger("substrate.client")


class Substrate:
    """The main SDK entry point.

    >>> client = Substrate()                    # uses env / config file
    >>> client = Substrate(token="mcp_...")     # explicit
    ...
    def __repr__(self) -> str:
        return f"Substrate(base_url={self._base_url!r}, token=***)"

    # ------------------------------------------------------------------------
    # Fluent builder entry points (immutable; each returns a fresh Launch).
    # See substrate.declarative.builder.Launch for the chain methods.
    # ------------------------------------------------------------------------
    ...
    def plan(
        ...
        *,
        ...
        from substrate.declarative.apply import plan as _plan
    ...
    def apply(
        ...
        *,
        ...
        """
        from substrate.declarative.apply import apply as _apply
    ...
    def destroy(
        ...
        *,
        ...
        """
        from substrate.declarative.apply import destroy as _destroy
    ...
    def run(
        ...
        *,
        ...
        _log.info(
            "substrate.run.selected_inventory",
...
def _build_manifest_from_kwargs(kwargs: dict[str, Any]) -> Manifest:
    """Translate `Substrate.launch(**kwargs)` flat kwargs into a Manifest.
    ...
    if "name" not in kwargs:
        raise ValueError("Substrate.launch(...) requires a `name` kwarg")
    ...
    if "image" in kwargs:
        from substrate.declarative.builder import _env_to_manifest
    ...
    if kwargs:
        raise TypeError(
            f"Substrate.launch(...) got unexpected kwargs: {sorted(kwargs)}"
        )
