...
`apply` / `plan` / `destroy` live on the `Substrate` client itself (not on
...
class Launch:
    ...
    @classmethod
    def from_manifest(cls, manifest: Manifest, *, client: Substrate | None = None) -> Launch:
    ...
    def launch(
        ...
        *,
        ...
        if self._client is None:
            raise RuntimeError(
                "Launch is detached from a client. Use Substrate().gpu(...) "
