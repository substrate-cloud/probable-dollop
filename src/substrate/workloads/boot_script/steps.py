...
and writes a per-step log to /var/log/substrate-boot/<step>.log plus a status
entry to /var/log/substrate-boot/manifest.json.
...
from substrate.workloads.secret import Secret, resolve_value

_LOG_DIR = "/var/log/substrate-boot"
...
def _log_block(step_id: str) -> tuple[str, str]:
    ...
    open_snip = (
        ...
        f"echo \"[substrate-boot] {step_id} starting at $(date -u +%FT%TZ)\" "
    ...
    close_snip = (
        ...
        f"echo \"[substrate-boot] {step_id} done rc=$_substrate_step_rc\" "
        f"| tee -a \"$_substrate_step_log\"\n"
...
class RunSystemdUnit(Step):
    ...
    def _body(self) -> str:
        ...
        return (
            f"cat > {unit_path} <<'SUBSTRATE_UNIT_EOF'\n{heredoc}SUBSTRATE_UNIT_EOF\n"
...
class IdleShutdown(Step):
    ...
    Note: OS shutdown does NOT stop Substrate billing — the instance must be
    ...
    def _body(self) -> str:
        ...
        script = (
            ...
            "  IDLE_FILE=/var/run/substrate-idle-since\n"
            "  if [ ! -f \"$IDLE_FILE\" ]; then date +%s > \"$IDLE_FILE\"; exit 0; fi\n"
            "  IDLE_SINCE=$(cat \"$IDLE_FILE\")\n"
            "  NOW=$(date +%s)\n"
            "  if [ $(( (NOW - IDLE_SINCE) / 60 )) -ge \"$IDLE_MIN\" ]; then\n"
            f"    {del_curl}"
            "    /sbin/shutdown -h now 'substrate idle shutdown'\n"
            "  fi\n"
            "else\n"
            "  rm -f /var/run/substrate-idle-since\n"
            "fi\n"
        )
        return (
            "cat > /usr/local/sbin/substrate-idle-check <<'SUBSTRATE_IDLE_EOF'\n"
            + script
            + "SUBSTRATE_IDLE_EOF\n"
            "chmod +x /usr/local/sbin/substrate-idle-check\n"
            "cat > /etc/cron.d/substrate-idle <<'CRON_EOF'\n"
            "*/5 * * * * root /usr/local/sbin/substrate-idle-check\n"
            "CRON_EOF\n"
        )
...
_PREAMBLE = (
    "#!/usr/bin/env bash\n"
    "# Generated by substrate-sdk BootScript. Do not edit on-host.\n"
    "set -euo pipefail\n"
    f"mkdir -p {_LOG_DIR}\n"
    f"exec > >(tee -a {_LOG_DIR}/boot.log) 2>&1\n"
    "echo \"[substrate-boot] starting at $(date -u +%FT%TZ)\"\n"
)
...
def epilogue() -> str:
    return "echo \"[substrate-boot] all steps completed at $(date -u +%FT%TZ)\"\n"
