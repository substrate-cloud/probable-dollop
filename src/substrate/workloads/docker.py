...
from substrate.workloads.base import HealthCheck
...
class DockerWorkload(BaseModel):
    ...
    def _resolve_envs(self) -> list[EnvVar]:
        ...
        for name, value in self.env.items():
            ...
            # The Substrate API persists this payload server-side (see plan doc §10.3).
    ...
    for pattern in _SECRET_LITERAL_PATTERNS:
        if pattern.search(value):
            raise ValueError(
                ...
                f"or persisted in the Substrate launch_configuration."
            )
