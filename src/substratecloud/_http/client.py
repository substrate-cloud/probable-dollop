...
- Map HTTP status codes to typed SubstrateCloudError subclasses.
...
from substratecloud._http.auth import bearer_header
...
from substratecloud._http.retries import (
    RetryPolicy,
...
_USER_AGENT = f"substratecloud-python/{__version__}"

_log = get_logger("substratecloud.http")


class HttpClient:
    """Shared sync+async transport for the SubstrateCloud MCP API.
        ...
        *,
        ...
        while True:
            ...
            try:
                ...
                _log.warning(
                    "substratecloud.http.transport_error",
                    method=method,
                    route=path,
                ...
                raise TransportError(
                    f"Transport error contacting SubstrateCloud: {exc}",
                    route=path,
                    request_id=request_id,
                ) from exc
...
def _decode(response: httpx.Response, route: str, request_id: str) -> Any:
    ...
    try:
        ...
        raise SubstrateCloudError(
            f"SubstrateCloud returned non-JSON body (status {response.status_code})",
...
def _http_error(response: httpx.Response, route: str, request_id: str) -> SubstrateCloudError:
    body: Any
    try:
        body = response.json()
    except ValueError:
