...
_log = get_logger("substratecloud.declarative.apply")
...
@dataclass
class Plan:
    """Output of `SubstrateCloud.plan(manifest)` — describes what apply would do."""
    ...
    def summary(self) -> str:
        ...
        lines.append("  Teardown     : substratecloud destroy <name> when the workload finishes")
...
def apply(
    ...
    *,
    ...
    if existing is not None:
        ...
        _log.warning(
            "substratecloud.apply.force_destroy_relaunch",
            name=manifest.name,
            instance_id=str(existing.id),
            drift=drift,
        )
...
def _find_existing_active(client: SubstrateCloud, manifest: Manifest) -> Instance | None:
...
def _estimate_inventory(
    client: SubstrateCloud, manifest: Manifest
    ...
    try:
        ...
        _log.warning("substratecloud.plan.inventory_estimate_failed", error=str(exc))
        return None, None, None
